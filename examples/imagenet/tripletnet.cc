...
LayerConf GenSliceConf(string name, vector<size_t>& slice_point) {
  LayerConf conf;
  conf.set_name(name);
  conf.set_type("singa_slice");

  SliceConf *slice = conf.mutable_slice_conf();
  slice->set_axis(0);
  for (size_t i = 0; i < slice_point.size(); i++)
    slice->add_slice_point(slice_point[i]);
  return conf;
}

LayerConf GenConcatConf(string name) {
  LayerConf conf;
  conf.set_name(name);
  conf.set_type("singa_concat");
  ConcatConf *concat = conf.mutable_concat_conf();
  concat->set_axis(0);
  concat->set_in_size(3);
  return conf;
}
...
FeedForwardNet CreateNet() {
  ...
  vector<size_t> slice_points;
  slice_points.push_back(64);
...
void TrainOneEpoch(FeedForwardNet &net, TRIPLET &data,
...
void TestOneEpoch(FeedForwardNet &net, TRIPLET &data,
                  std::shared_ptr<Device> device, int epoch, string bin_folder,
